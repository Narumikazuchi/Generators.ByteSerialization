...
public partial class SerializableGenerator
{
                                                ...
                                                String indent)
    {
        ...
        builder.AppendLine($"{indent}static Int32 Narumikazuchi.Serialization.Bytes.IByteSerializable<{symbol.Name}>.Serialize(Span<Byte> buffer, {symbol.Name} value)");
        builder.AppendLine($"{indent}{{");
        indent += "    ";

        builder.AppendLine($"{indent}Int32 pointer = 0;");
        builder.AppendLine($"{indent}Int32 written;");
                                                  ...
                                                  String indent)
    {
        foreach (IFieldSymbol field in fields)
        {
            ...
            {
                builder.AppendLine($"{indent}written = Narumikazuchi.Serialization.Bytes.ByteSerializer.Serialize<{field.Type.Name}, {strategyType.ToDisplayString()}>(buffer[pointer..], value.{target.Name});");
                builder.AppendLine($"{indent}pointer += written;");
            }
            ...
            else
            {
                ...
                if (isSerializable)
                {
                    builder.AppendLine($"{indent}written = Narumikazuchi.Serialization.Bytes.ByteSerializer.Serialize(buffer[pointer..], value.{target.Name});");
                    builder.AppendLine($"{indent}pointer += written;");
                }
                         ...
                         field.Type.TypeKind is TypeKind.Enum)
                {
                    ...
                    {
                        builder.AppendLine($"{indent}written = Narumikazuchi.Serialization.Bytes.Strategies.Int32Strategy.Serialize(buffer)[pointer..], (Int32)value.{target.Name});");
                        builder.AppendLine($"{indent}pointer += written;");
                    }
                    else
                    {
