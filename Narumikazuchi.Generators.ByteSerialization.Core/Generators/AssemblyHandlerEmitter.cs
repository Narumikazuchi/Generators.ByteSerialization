...
static public class AssemblyHandlerEmitter
{
                            ...
                            Compilation compilation)
    {
        ...
        ImmutableArray<ITypeSymbol> allTypes = candidates.SelectMany(type => type.AllInterfaces.Where(@interface => @interface.IsSerializationHandler())
                                                                                               .Select(@interface => @interface.TypeArguments[0]))
                                                         .Concat(types)
                                                         .ToImmutableArray();
        StringBuilder typeIds = new();
        typeIds.Append($"        this.Types.Add((({GlobalNames.ISerializationHandler(nameof(String))})this).TypeIdentifier, typeof(String));");
        foreach (ITypeSymbol type in allTypes.Where(type => type is IArrayTypeSymbol ||
                                                            (type is INamedTypeSymbol namedType &&
                                                            !namedType.IsOpenGenericType())))
        {
            typeIds.AppendLine();
            typeIds.Append($"        this.Types.Add((({GlobalNames.ISerializationHandler(type)})this).TypeIdentifier, typeof({type.ToFrameworkString()}));");
        }
...
{typeIds}
    ...
    public {GlobalNames.NAMESPACE}.MirroredMap<{GlobalNames.NAMESPACE}.TypeIdentifier, Type> Types {{ get; }} = new();
}}";
