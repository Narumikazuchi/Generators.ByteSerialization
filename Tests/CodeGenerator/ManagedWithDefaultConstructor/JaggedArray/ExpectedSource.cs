...
static public class ExpectedSource
{
    ...
    public const String HANDLER_FILENAME = @"Narumikazuchi.Generators.ByteSerialization.Core\Narumikazuchi.Generators.ByteSerialization.Generators.SerializableGenerator\Narumikazuchi.Generated.Internals.ByteSerialization.Handler.Array[Array[Person+1]+1].g.cs";
                ...
                if (buffer[pointer++] == 0x1)
                {
                    ...
                    for (var _var6 = 0; _var6 < _var5; _var6++)
                    {
                        ...
                        if (buffer[pointer++] == 0x1)
                        {
                            ...
                            var _var15 = Unsafe.As<Byte, DateOnly>(ref MemoryMarshal.GetReference(buffer[pointer..]));
                            pointer += Unsafe.SizeOf<DateOnly>();
                            var _var16 = Unsafe.As<Byte, Guid>(ref MemoryMarshal.GetReference(buffer[pointer..]));
