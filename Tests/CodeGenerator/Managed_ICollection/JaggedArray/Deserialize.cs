...
public class Application
{
    ...
    {
        return ByteSerializer.Deserialize<List<Person>[][]>(buffer, out _);
    }
}";
...
public class Application
{
    static public unsafe UInt32 Run(Byte[] buffer)
    {
        ...
        {
            result = ByteSerializer.Deserialize<List<Person>[][]>(pointer, out _);
        }
        return result;
    }
}";
...
public class Application
{
    ...
    {
        return ByteSerializer.Deserialize<List<Person>[][]>(stream, out _);
    }
}";
...
public class Application
{
    ...
    {
        return ByteSerializer.Deserialize<TStream, List<Person>[][]>(stream, out _);
    }
}";
...
public class Application
{
    ...
    {
        _ = await ByteSerializer.DeserializeAsynchronously<List<Person>[][]>(stream, cancellationToken);
    }
}";
...
public class Application
{
    static public async Task Run<TStream>(TStream stream, CancellationToken cancellationToken)
        where TStream : IReadableStream
    {
        _ = await ByteSerializer.DeserializeAsynchronously<TStream, List<Person>[][]>(stream, cancellationToken);
    }
}";
