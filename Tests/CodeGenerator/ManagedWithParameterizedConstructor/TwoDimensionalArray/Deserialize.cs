...
public class Application
{
    ...
    {
        return ByteSerializer.Deserialize<Product[,]>(buffer, out _);
    }
}";
...
public class Application
{
    static public unsafe UInt32 Run(Byte[] buffer)
    {
        ...
        {
            result = ByteSerializer.Deserialize<Product[,]>(pointer, out _);
        }
        return result;
    }
}";
...
public class Application
{
    ...
    {
        return ByteSerializer.Deserialize<Product[,]>(stream, out _);
    }
}";
...
public class Application
{
    ...
    {
        return ByteSerializer.Deserialize<TStream, Product[,]>(stream, out _);
    }
}";
...
public class Application
{
    ...
    {
        _ = await ByteSerializer.DeserializeAsynchronously<Product[,]>(stream, cancellationToken);
    }
}";
...
public class Application
{
    static public async Task Run<TStream>(TStream stream, CancellationToken cancellationToken)
        where TStream : IReadableStream
    {
        _ = await ByteSerializer.DeserializeAsynchronously<TStream, Product[,]>(stream, cancellationToken);
    }
}";
